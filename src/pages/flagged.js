  ...
  return <Grid container spacing={2}>
    {data.queryPost.map(post =>
      <Grid item xs={12} sm={6} md={4} lg={3} key={post.text}>
        <PostCard author={post.createdby.username} text={post.text} isApproved={false} postID={post.id} likes={post.likes} flags={post.flags} flagCount={post.flags.length} tags={post.tags} updateCache={updateCache}/>
      </Grid>
    )}
  </Grid>;
}
